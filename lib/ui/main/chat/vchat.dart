...
import 'package:wideal/model/server/chatGet.dart';
import 'package:wideal/model/server/chatSend.dart';
import 'package:wideal/model/server/chatUsers.dart';
import 'package:wideal/widget/ICard31FileCaching.dart';
import 'package:wideal/widget/InputFieldArea3.dart';
import 'package:wideal/widget/easyDialog2.dart';
import 'package:wideal/widget/ibackground4.dart';
...
class _VendorChatScreenState extends State<VendorChatScreen> {
  ...
  onSendText() {
    ...
    vChatSend(
        _userSelected, account.token, editController.text, _success, _error);
  }
  ...
  _onBack() {
    if (_state == "root") return widget.callback("home");
    if (_state == "view") {
      _state = "root";
      chatUsers(account.token, _successUsers, _error);
    }
        ...
        child: Scaffold(
            ...
            body: Directionality(
                ...
                child: Stack(
                  children: <Widget>[
                    IBackground4(
                        width: windowWidth, colorsGradient: colorsGradient),
                    if (_state == "root")
                      Container(
                    ...
                    if (_state == "view")
                      buttonBackUp(
                          _onBack, MediaQuery.of(context).padding.top + 15),
                    if (_state == "root") buttonBack(_onBack),
  ...
  _body2() {
    ...
    for (var _data in _this) {
      ...
      if (now != last) {
        ...
        list.add(
          Text(now, style: theme.text14boldWhite, textAlign: TextAlign.center),
        );
        list.add(SizedBox(
          height: 10,
        ));
  ...
  callbackReload() {
    ...
    if (_state == "root") chatUsers(account.token, _successUsers, _error);
  }
