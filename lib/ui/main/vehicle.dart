...
class _VehicleScreenState extends State<VehicleScreen> {
  ...
  _saveVehicleInformation() {
    if (_selectVehicleId.isEmpty || _selectColorId.isEmpty) return;
    widget.callback(
        "${strings.get(245)}: $_selectVehicleId, ${strings.get(246)}: "
  ...
  _getList() {
    ...
    list.add(Container(
      ...
      child: Text(
        ...
        style: theme.text14,
        textAlign: TextAlign.center,
      ), // "Help us locate your vehicle when you arrive.",
    ));
    list.add(SizedBox(
      height: 30,
    ));
    ...
    list.add(Container(
      ...
      child: IInputField2(
        hint: strings.get(293), // Vehicle Number",
        icon: Icons.chat,
  ...
  _oneCar(String image, String name) {
    ...
    if (_selectVehicleId == name) _select = Colors.red;
    return InkWell(
        onTap: () {
  ...
  _color(Color _color, String text) {
    var _select = Colors.transparent;
    if (_selectColorId == text) _select = Colors.red;
    return InkWell(
        onTap: () {
          _selectColorId = text;
          setState(() {});
