...
addOrdersDetails(List<Widget> list, List<OrdersData> _data, String idOrder,
    double windowWidth) {
  list.add(SizedBox(
    height: 20,
  ));
  ...
  for (var item2 in _data)
    if (item2.orderid == idOrder) {
      ...
      for (var item in item2.ordersdetails) {
        if (item.count != 0) {
          ...
          list.add(Container(
            margin: EdgeInsets.only(left: 16, right: 15),
            child: Text(
                "${basket.makePriceString(item.foodprice)} x ${item.count} = ${basket.makePriceString(item.foodprice * item.count)}",
          ...
          list.add(Container(
            ...
            child: Text(
                "${basket.makePriceString(item.extrasprice)} x ${item.extrascount} = ${basket.makePriceString(item.extrasprice * item.extrascount)}",
      ...
      // fee
      var fee = item2.fee;
      if (item2.percent == '1') fee = item2.fee / 100 * subtotal;
      ...
      list.add(itemBasketText(
          strings.get(95), basket.makePriceString(tax), false)); // "Taxes",
      list.add(SizedBox(
        height: 5,
      ));
      list.add(itemBasketText(strings.get(96),
          basket.makePriceString(item2.total), true)); // "Total",
    }
}
