...
import 'package:wideal/model/server/addressDelete.dart';
import 'package:wideal/model/server/addressGet.dart';
import 'package:wideal/model/server/addressSave.dart';
import 'package:wideal/widget/ICard35FileCaching.dart';
...
import 'package:wideal/ui/main/map2.dart';
...
class _AddressScreenState extends State<AddressScreen> {
  ...
  _getList() {
    ...
    list.add(
      Container(
        alignment: Alignment.center,
        child: Text(
          "${strings.get(279)}: $_deliveryLatitude \n${strings.get(280)} $_deliveryLongitude",
          style: theme.text16,
        ), // Latitude - Longitude
      ),
    );
    ...
    if (editControllerAddress.text.isEmpty) _color = theme.colorGrey;
        ...
        child: Row(
          children: [
            Expanded(
                child: Container(
              height: 0.3,
              color: theme.colorDefaultText.withAlpha(120),
            )),
    ...
    list.add(
      Container(
        alignment: Alignment.center,
        child: IButton3(
          pressButton: _clickSelectAddressButton,
    ...
    if (address.isNotEmpty) {
      for (var _data in address) {
        ...
        if (_data.type == "work") _type = strings.get(282); // work
        if (_data.type == "other") _type = strings.get(283); // Other
        list.add(Container(
            ...
            child: ICard35FileCaching(
              ...
              title: _data.text,
              type: _type,
              work: (_data.defaultAddress == "true")
                  ? strings.get(287)
                  : "", // default
              titleStyle: theme.text14bold,
              colorProgressBar: Colors.white,
              text:
                  "${strings.get(279)}: ${_data.lat} ${strings.get(280)} ${_data.lng}", // Latitude - Longitude
      ...
      for (var _data in address)
        if (_data.selected) _color = theme.colorPrimary;
      list.add(Container(
  ...
  _clickSelectAddressButton() {
    Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                MapInfoScreen(callback: (double lat, double lng, String addr) {
