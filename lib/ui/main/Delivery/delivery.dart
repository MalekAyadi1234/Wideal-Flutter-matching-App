...
import 'package:wideal/model/server/addressGet.dart';
import 'package:wideal/ui/main/Delivery/payments.dart';
import 'package:wideal/ui/main/Delivery/widgets.dart';
import 'package:wideal/widget/skinRoute.dart';
import 'address.dart';
...
import 'package:wideal/ui/main/vehicle.dart';
import 'package:wideal/widget/easyDialog2.dart';
import 'package:wideal/widget/ibutton3.dart';
import 'package:wideal/widget/iinkwell.dart';
...
class _DeliveryScreenState extends State<DeliveryScreen>
  ...
  _pressContinueButton() async {
    ...
    if (stage == 3)
      pay(_currVal, context, _waits, editControllerPhone.text, _openDialog,
          openDialog);
    if (stage == 2) {
      stage = 3;
      _redraw();
    }
    if (stage == 4) Navigator.pop(context);
    if (stage == 1) {
      print("User pressed \"Continue\" button");
      print(
          "phone: ${editControllerPhone.text}, comments: ${editControllerComments.text}");
      ...
      if (_timeArrivedInit) text = "$text $_timeArrived";
      ...
      text =
          "$text - ${strings.get(85)}: ${editControllerComments.text}"; // comments
  ...
  _onBackClick() {
    if (stage == 1 || stage == 4) return Navigator.pop(context);
    if (stage == 2) stage = 1;
    if (stage == 3) stage = 2;
    setState(() {});
  }
  ...
  _getList() {
    ...
    if (stage == 1) {
      ...
      if (basket.curbsidePickup)
        list.add(
            buttonPressOrContinue(windowWidth, _pressContinueButton, stage));
      else if (!_distanceToMax)
        list.add(
            buttonPressOrContinue(windowWidth, _pressContinueButton, stage));
    ...
    if (stage == 2) // information summary
      informationSummaryScreen(list, windowWidth, editControllerAddress.text,
          _couponEnable, _pressContinueButton, stage);

    if (stage == 3) {
      // payments gateway list
      paymentsGatewayList(
          list, windowWidth, _currVal, _setCurVal, _pressContinueButton, stage);
    }

    if (stage == 4) finishWidget(list, windowWidth, windowHeight);
  ...
  _body(List<Widget> list) {
    ...
    if (!basket.curbsidePickup && appSettings.delivering == "true") {
      ...
      var _text =
          "Latitude: ${_deliveryLatitude.toStringAsFixed(6)}, Longitude: ${_deliveryLongitude.toStringAsFixed(6)}";
      list.add(Container(
          margin: const EdgeInsets.fromLTRB(20, 0, 20, 20),
          child: Text(
            (_deliveryAddressInit) ? _text : strings.get(183),
            style: theme.text14,
          ) // "no address",
          ));
      ...
      if (_deliveryAddressInit) {
        ...
        deliveryDistanceText(
            ...
            editControllerAddress
                .text); // from address - to address and distance
      }
          ...
          Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (context) => AddressScreen(
                          callback: (String text, double lat, double lng) {
                        _deliveryAddressInit = true;
                        ...
                        pref.set(Pref.deliveryLatitude,
                            _deliveryLatitude.toString());
                        pref.set(Pref.deliveryLongitude,
                            _deliveryLongitude.toString());
                        editControllerAddress.text = _deliveryAddress;
                        editControllerAddress.selection =
                            TextSelection.fromPosition(
                          TextPosition(
                              offset: editControllerAddress.text.length),
                        );
                        _initDistance();
                        setState(() {});
                      })));
    ...
    if (appSettings.deliveringDate == "true")
      deliveryDate.changeDeliveryDate(list, windowWidth, (double value) {
        setState(() {
          _show = value;
        });
      }, (Widget body) {
  ...
  openDialogTime() {
    ...
    var widget = CupertinoDatePicker(
      ...
      onDateTimeChanged: (DateTime picked) {
        ...
        if (picked.hour.toString().length == 1) h = "0$h";
        var m = picked.minute.toString();
        if (picked.minute.toString().length == 1) m = "0$m";
        _timeArrived = "$h:$m";
        dprint("$_timeArrived");
      },
      use24hFormat: true,
...
//      minuteInterval: 1,
  ...
  _onCouponEnter(String value) {
    ...
    if (!_couponEnable) basket.setCoupon(null);
    setState(() {});
  }
}
