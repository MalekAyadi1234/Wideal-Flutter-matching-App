...
import 'package:wideal/widget/basic/isearch.dart';
...
class _MapInfoScreenState extends State<MapInfoScreen> {
  ...
  _backButtonPress() {
    if (_currentPos != null)
      widget.callback(_currentPos!.latitude, _currentPos!.longitude,
          (_textAddress.isNotEmpty) ? _textAddress : _searchValue);
  ...
  _onMapTap(LatLng pos) async {
    PlacesSearchResponse response = await places.searchNearbyWithRadius(
        new Location(lat: pos.latitude, lng: pos.longitude), 20);
    _textAddress = "";
    if (response.results.isNotEmpty) {
      for (var item in response.results)
        if (item.vicinity != null) if (item.vicinity!.length >
            _textAddress.length) _textAddress = item.vicinity!;
    }
    setState(() {});
  }
      ...
      _searchResult.add(Stack(
        children: [
          Container(
              ...
              child: Text(
                ret.formattedAddress != null ? ret.formattedAddress! : "",
            ...
            body: Directionality(
                ...
                child: Container(
                  ...
                  Align(
                    ...
                    child: Container(
                        margin:
                            EdgeInsets.only(bottom: 10, right: 70, left: 70),
                        child: Container(
                            ...
                            ) // ""Select place on map or find address on Search",",
                            )),
                  ),
                ])))));
  }
