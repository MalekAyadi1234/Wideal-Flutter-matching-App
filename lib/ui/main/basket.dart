...
import 'package:wideal/widget/BasketItem.dart';
import 'package:wideal/widget/ilist1.dart';
...
class _BasketScreenState extends State<BasketScreen>
  ...
  _pressCheckoutButton() {
    ...
    if (basket.basket.isEmpty) return;
    if (!account.isAuth())
    ...
    return Column(
        ...
        children: <Widget>[
          ...
          Container(
              ...
              child: IButton3(
                text: strings.get(88),
                textStyle: theme.text14boldWhite, // Continue Shopping
  ...
  List<Widget> _bottomBar() {
    ...
    if (basket.perkm == '1')
      list.add(itemBasketText(
          strings.get(94),
          "${basket.makePriceString(basket.fee)} ${strings.get(300)} ${appSettings.distanceUnit}",
      ...
      if (restaurant.minimumAmount != 0) {
        if (basket.getTotal(false) < restaurant.minimumAmount) {
          list.add(Container(
            ...
            child: Text(
                "${strings.get(294)}: ${basket.makePriceString(restaurant.minimumAmount)}",
                style: theme.text16Red), // Minimum order amount
          ));
    ...
    list.add(Container(
      margin: EdgeInsets.only(left: 30, right: 30),
      child: IButton3(
        text: strings.get(97), textStyle: theme.text14boldWhite,
  ...
  List<Widget> _getDataActive() {
    ...
    if (restaurant != null) {
      ...
      list.add(Container(
        height: 0.5,
        color: theme.colorDefaultText.withAlpha(100),
      ));
      list.add(SizedBox(
        height: 10,
      ));
  ...
  _item(DishesData item) {
    ...
    if (item.discountprice != 0) _total = item.discountprice * item.count;
    ...
    if (_count != 0)
      _title =
          "${item.name} ${strings.get(197)} $_count ${strings.get(198)}"; // name and 3 items
