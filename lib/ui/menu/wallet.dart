...
import 'package:wideal/ui/login/needAuth.dart';
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  _pressTopUpNow() async {
    dprint(
        "User pressed Top Up Now. num=${editControllerCount.text} _currVal=$_currVal");
    total = 0;
    try {
      total = double.parse(editControllerCount.text);
    } catch (ex) {}
    if (total == 0) return openDialog(strings.get(217)); // Enter the amount
    ...
    if (_currVal == 3) {
      ...
      var t = _total.toInt();
      _razorpayModel.openCheckout(t.toString(), "", "", razorpayCompanyName,
          code, _onSuccess, _onError);
    }
    if (_currVal == 7) {
      // payStack
      var paystack = PayStackModel();
      var ret = await paystack.handleCheckout(total, account.email, context);
      if (ret != null) _onSuccess(ret);
    }
    if (_currVal == 5) {
      // paypal
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => PaypalPayment(
              ...
              sandBoxMode:
                  homeScreen.mainWindowData!.payments.payPalSandBoxMode,
              onFinish: (w) {
                _onSuccess("PayPal: $w");
              }),
        ),
    ...
    if (_currVal == 8) {
      ...
      try {
        ...
        _waits(false);
        if (ret != null) _onSuccess("YandexKassa: $ret");
      } catch (ex) {
        _waits(false);
      }
    }
    ...
    if (_currVal == 12) {
      ...
      var ret = await FlutterWaveModel().handleCheckout(
          toDouble(total.toStringAsFixed(appSettings.symbolDigits)),
          account.email,
          code,
          context,
          account.phone,
  ...
  _onSuccess(String id) {
    walletTopUp(account.token, total.toStringAsFixed(appSettings.symbolDigits),
        id, _successTopUp, _error);
  }
    ...
    return Scaffold(
        ...
        body: Directionality(
            ...
            child: Stack(
              children: <Widget>[
                skinHeader(
                    context, widget.onBack, strings.get(206)), // "Wallet",
  ...
  _body2() {
    ...
    list.add(Text(
      strings.get(207),
      style: theme.text14,
    )); // "Top up and enjoy these benefits",
    list.add(SizedBox(
      height: 20,
    ));

    list.add(_row(strings.get(208),
        strings.get(209))); //  "Easy payment", - "Order food with 1 click",
    list.add(SizedBox(
      height: 20,
    ));
    list.add(_row(strings.get(210),
        strings.get(211))); //  "Cashless delivery", - "Avoid the cash hassle",
    list.add(SizedBox(
      height: 20,
    ));
    list.add(_row(
        strings.get(212),
        strings.get(
            213))); //  "Instant refunds", - "When you need to cancel an order",
    list.add(SizedBox(
      height: 30,
    ));
    ...
    var _text = (appSettings.rightSymbol == "true")
        ? "${account.walletBalance.toStringAsFixed(appSettings.symbolDigits)}${appSettings.currency}"
    ...
    if (!_topUpStart) {
      ...
      var _text = (appSettings.rightSymbol == "true")
          ? "${100.toStringAsFixed(appSettings.symbolDigits)}${appSettings.currency}"
      ...
      if (homeScreen.mainWindowData!.payments.stripeEnable == "true") {
        list.add(_item(
            "assets/payment2.png", 4, strings.get(192))); // "Visa, Mastercard",
        list.add(SizedBox(
          height: 10,
        ));
      }
      ...
      if (homeScreen.mainWindowData!.payments.yandexKassaEnable == "true") {
        list.add(
            _item("assets/payment8.png", 8, strings.get(265))); // Yandex kassa
        list.add(SizedBox(
          height: 10,
        ));
      }
      if (homeScreen.mainWindowData!.payments.instamojoEnable == "true") {
        list.add(
            _item("assets/payment9.png", 9, strings.get(266))); // "Instamojo",
        list.add(SizedBox(
          height: 10,
        ));
      }
      if (homeScreen.mainWindowData!.payments.flutterWaveEnable == "true") {
        list.add(
            _item("assets/payment12.png", 12, "FlutterWave")); // FlutterWave
        list.add(SizedBox(
          height: 10,
        ));
      }
      ...
      list.add(Container(
          ...
          margin: EdgeInsets.only(
              left: windowWidth * 0.1, right: windowWidth * 0.1),
          child: IButton3(
            text: strings.get(216), // "Top Up Now",
            color: theme.colorPrimary,
