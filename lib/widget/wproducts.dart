...
import 'package:wideal/widget/basic/search.dart';
import '../main.dart';
...
dishList2(
    List<Widget> list,
    List<DishesData> _mostPopular,
    BuildContext context,
    Function(String id, String heroId) _onMostPopularClick,
    double windowWidth,
  ...
  for (var item in _mostPopular) {
    ...
    if (restaurantSearchValue != "0" &&
        item.restaurant != restaurantSearchValue) continue;
    ...
    if (searchByCategory.isNotEmpty && item.category != searchByCategory) {
      ...
      for (var temp in treeViewCategory) {
        for (var temp2 in temp) {
          ...
          if (start && temp2.id == item.category) search = true;
        }
      }
      if (!search) continue;
    }
    if (first) {
      ...
      if (index == 0 && size > 2) _height = constHeight / 2 - 5;

      if (index == size - 1 && size > 2) _height = constHeight / 2 - 5;
      ...
      _childs.add(Container(
        ...
        child: _card32item(
            item, windowWidth, _height, _onMostPopularClick, onAddToCartClick),
      ));
      y1Start += _height + 10;
    } else {
      _height = constHeight;
      ...
      first = true;
      var margin =
          EdgeInsets.only(left: windowWidth / 2 + 5, top: y2Start, right: 10);
      if (strings.direction == TextDirection.rtl)
        margin =
            EdgeInsets.only(right: windowWidth / 2 + 5, top: y2Start, left: 10);
  ...
  if (y2Start == 10) y2Start = _height;
  if (size == 1 || size == 2) y2Start = constHeight;
  if (_childs.length != 0)
    list.add(Container(
      width: windowWidth,
      height: y2Start + 20,
...
_card32item(
  ...
  return ProductsTileListV2(
    ...
    text: item.name,
    text3:
        (theme.multiple) ? item.restaurantName : getCategoryName(item.category),
    ...
    discountprice: (item.discountprice != 0)
        ? basket.makePriceString(item.discountprice)
        : "",
    //
...
dishListOneInLine(
  ...
  for (var item in productItems) {
    ...
    list.add(Container(padding: EdgeInsets.only(left: 5, right: 5), child: t2));
  }
}

horizontalCategoriesCircleRestaurant(
    List<CategoriesData> cat,
    double windowWidth,
    Function(String id, String hero, String image) _onCategoriesClick) {
  List<Widget> list = [];
  list.add(SizedBox(
    width: 10,
  ));
...
horizontalCategoriesCircle(double windowWidth,
    Function(String id, String hero, String image) _onCategoriesClick) {
  List<Widget> list = [];
  list.add(SizedBox(
    width: 10,
...
horizontalCategories(double windowWidth,
    Function(String id, String hero, String image) _onCategoriesClick) {
  List<Widget> list = [];
  list.add(SizedBox(
    width: 10,
...
horizontalCategoriesRestaurant(List<CategoriesData> cat, double windowWidth,
    Function(String id, String hero, String image) _onCategoriesClick) {
  List<Widget> list = [];
  list.add(SizedBox(
    width: 10,
...
_findPoint2(List<CategoriesData> cat, String _currentCategoryId,
    List<List<CategoriesData>> tree) {
  for (var item in cat) {
    if (item.id == _currentCategoryId) {
      tree.insert(0, _addLine(item.parent, cat, ""));
      if (item.parent != '0') tree = _findPoint2(cat, item.parent, tree);
      return tree;
    }
  }
  return tree;
...
horizontalCategoriesCircleRestaurantV2(
    List<Widget> listTopLevel,
    List<CategoriesData> cat,
    List<DishesData> prod,
    double windowWidth,
    Function(String id, String hero, String image) _onCategoriesClick,
  ...
  for (var item in treeViewCategory) {
    _getLineLevel(
        ...
        windowWidth,
        _onCategoriesClick,
        (currentLevel != treeViewCategory.length) ? 0.7 : 1.0,
        treeViewCategory.length,
        _currentCategoryId);
    currentLevel++;
  }
}

_buttonAll(double windowWidth, double squareScale, double height,
    Function(String id, String hero, String image) _onCategoriesClick) {
  return Stack(
    children: <Widget>[
      Container(
...
_getLineLevel(
  ...
  for (var item in cat) {
    ...
    Widget child = ICard30FileCaching(
      ...
      image: item.image,
      id: item.id,
      textStyle: (squareScale == 0)
          ? theme.text18boldPrimaryUI
          : TextStyle(
              color: theme.colorDefaultText,
    ...
    if (appSettings.categoryCardCircle != "true")
      child = ICard12FileCaching(
        ...
        text: item.name,
        width:
            (windowWidth * appSettings.categoryCardWidth / 100) * squareScale,
        height: height * squareScale,
        image: item.image,
        id: item.id,
        textStyle: (squareScale == 0)
