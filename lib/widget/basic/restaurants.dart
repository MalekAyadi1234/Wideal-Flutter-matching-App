...
import 'package:wideal/widget/ipromotion.dart';
...
isInCityProduct(
  ...
  for (var item in nearYourRestaurants)
    if (product.restaurant == item.id) {
      if (item.city.isEmpty) return true;
      var cities = item.city.split(',');
      var currentCity = pref.get(Pref.city);
      if (currentCity.isEmpty) return true;
      for (var city in cities) if (city == currentCity) return true;
    }
  return false;
}
