...
import 'package:wideal/model/server/addToBasket.dart';
import 'package:wideal/model/server/basketReset.dart';
import 'package:wideal/model/server/deleteFromBasket.dart';
import 'package:wideal/model/server/getBasket.dart';
import 'package:wideal/model/server/mainwindowdata.dart';
import 'package:wideal/model/server/setCountinBasket.dart';
...
class Basket {
  ...
  bool restaurantCheck(String id) {
    for (var item in basket)
      if (item.count != 0) if (item.restaurant != id) return false;
    return true;
  }

  bool dishInBasket(String id) {
    for (var item in basket)
      if (item.count != 0) if (item.id == id) return true;
    return false;
  }

  int inBasket() {
    var lenght = 0;
    for (var item in basket) if (item.count != 0) lenght++;
    return lenght;
  }
  ...
  delete(DishesData item) {
    deleteFromBasket(account.token, orderid, item.id, () {}, (String _) {});
  }

  add(DishesData item) {
    // tax
    if (item.ver == '2') taxes = item.tax;
    //
    var _addedName = "";
    // if (item.variants != null)
    for (var item in item.variants) if (item.select) _addedName = item.name;
    //
    ...
    addToBasket(
      ...
      perkm = _perkm;
      orderid = id;
      for (var item in basket) item.delivered = true;
    }, (String _) {});
  }
  ...
  deleteFrmBasket(String id) {
    ...
    if (_current != null) basket.remove(_current);
  }
  ...
  double getTotal(bool needCoupons) {
    ...
    if (_curbsidePickup == "true" || curbsidePickup) _fee = 0;
  ...
  _getSubTotal() {
    double _total = 0;
    for (var item in basket) _total += getItemPrice(item);
    return _total;
  }
  ...
  getItemPrice(DishesData? item) {
    ...
    if (item.discountprice != 0) t = item.discountprice * item.count;
    // if (item.extras != null)
    for (var ex in item.extras) if (ex.select) t += (ex.price * item.count);
    return t;
  }
  ...
  getItemPriceText(DishesData item) {
    ...
    if (item.discountprice != 0) t = item.discountprice * item.count;
    var text =
        "${makePriceString(item.price)} x ${item.count} = ${makePriceString(t)}";
    // if (item.extras != null)
  ...
  _getItemPriceDEBUG(DishesData item) {
    ...
    for (var ex in item.extras)
      if (ex.select) {
        t += (ex.price * item.count);
        td =
            "$td and Extras ${ex.id}:${ex.name} price*count (${ex.price}*${item.count}=${ex.price * item.count})";
      }
    return "$td. TOTAL: $t";
  }
      ...
      if (_total > _coupon!.amount) {
        ...
        for (var food in basket) {
          ...
          if (_coupon!.allRestaurants == '1') {
            ...
            if (_coupon!.allCategory != '1' &&
                !_coupon!.categoryList.contains(food.category)) {
              priceCoupon = price;
              dprint(
                  "getSubTotal not present in category list=${_coupon!.categoryList} need=${food.category}");
            } else
              dprint(
                  "getSubTotal present in category list=${_coupon!.categoryList} need=${food.category}");

            if (_coupon!.allFoods != '1' &&
                !_coupon!.foodsList.contains(food.id)) {
              priceCoupon = price;
              dprint(
                  "getSubTotal not present in foods list=${_coupon!.foodsList} need=${food.id}");
            } else
              dprint(
                  "getSubTotal present in foods list=${_coupon!.foodsList} need=${food.id}");
          } else {
          ...
          if (priceCoupon != price)
            dprint(
                "getSubTotal food ${food.id}:${food.name} IN ACTION. ${_getItemPriceDEBUG(food)} WITH COUPON=${_couponCalculateDEBUG(price)}");
          else {
            dprint(
                "getSubTotal food ${food.id}:${food.name} NO IN ACTION. ${_getItemPriceDEBUG(food)}");
            couponComment =
                "$couponComment${strings.get(262)} ${food.name} ${strings.get(263)}\n"; // Food does not participate in the promotion",
          }
          total += priceCoupon;
        }
        if (total != _total) if (_coupon!.inpercents != '1')
          total = _total - _coupon!.discount;
        return total;
      } else {
        couponComment =
            "${strings.get(264)} ${_coupon!.amount}\n"; // "The minimum purchase amount must be",
      }
    }
    return _total;
  }
  ...
  createOrder(
    ...
    _paymentid = id;
    for (var item in basket) item.delivered = false;
    basketReset(account.token, () {
      ...
      addToBasket(
          ...
          lng,
          curbsidePickup,
          couponName, (String id, String _fee, String percent, String _perkm) {
        fee = double.parse(_fee);
        _percentage = percent;
        perkm = _perkm;
        orderid = id;
